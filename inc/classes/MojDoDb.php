...
define('DO_TABLE_PROFILES', '`Profiles`');
...
class MojDoDb extends MojDoMistake
{
    ...
    */
    function MojDoDb()
    {
        ...
        if (empty($GLOBALS['moj_db_link'])) {
            $this->connect();
            $GLOBALS['gl_db_cache'] = array();
            $GLOBALS['moj_db_param'] = array();
        } else {
            $this->link = $GLOBALS['moj_db_link'];
        }

        if(empty($GLOBALS['moj_db_param']))
            $GLOBALS['moj_db_param'] = new MojDoParams($this);

        $this->oParams = $GLOBALS['moj_db_param'];
    }
    ...
    function res($query, $error_checking = true)
    {
        ...
        if (isset($GLOBALS['moj_profiler'])) $GLOBALS['moj_profiler']->beginQuery($query);
        ...
        if (isset($GLOBALS['moj_profiler'])) $GLOBALS['moj_profiler']->endQuery($res);
...
EOJ;
    ...
    function getDbCacheObject ()
    {
        if ($this->oDbCacheObject != null) {
            ...
            $this->oDbCacheObject = moj_instance ('MojDoCache'.$sEngine);
            if (!$this->oDbCacheObject->isAvailable())
                $this->oDbCacheObject = moj_instance ('MojDoCacheFile');
            return $this->oDbCacheObject;
        }
    }
